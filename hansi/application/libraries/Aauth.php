...
public function is_loggedin(){
	...
	if( $this->CI->session->userdata('loggedin') ){ return true;}
...
public function is_group_allowed($perm_par, $group_par=false){
	...
	// checks current user's all groups
		...
		if( $this->is_admin( $this->CI->session->userdata('id')) or
			$this->is_group_allowed($perm_id, $this->config_vars['public_group'])){return true;}
...
public function send_pm( $sender_id, $receiver_id, $title, $message ){
	...
	// Disabled Self PM Ban from AAUTH
...
public function assign_company($uid,$gid){
	$query="INSERT INTO aauth_company (`user`,`group`) values($uid,$gid) ON DUPLICATE KEY UPDATE `group`=$gid";
	return $this->CI->db->query($query);
}
	
public function get_company($uid=null){
	
	if($uid==null){
		$uid=$this->CI->session->userdata('id');
	}
	$this->CI->db->where('user',$uid);
	$res=$this->CI->db->get('aauth_company');
